...
import type { CompanyProductivityResult, MonthlyPoint } from '../types/report'
import { getCompanyProductivity, getDepartmentProductivity } from '../services/dashboard'
import { useAuth } from '../hooks/useAuth'
import { ErrorState, LoadingState, PageHeader, ProgressBar, tableStyle, tdStyle, thStyle } from '../components/ui/common'
...
export default function Productivity() {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      try {
        ...
        else setDepartments(await getDepartmentProductivity())
      } catch {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      <PageHeader title="Produtividade" subtitle="Indicadores operacionais" />
      ...
      {authUser?.role === 'EMPLOYEE' && (
        <p style={{ color: 'var(--clop-gray)', fontSize: '0.9rem' }}>A sua produtividade individual está disponível no dashboard.</p>
      )}
    </div>
  )
}